...
from train_util import Logger, compute_trajectory_metrics
...
def plot(name):
    ...
    df.plot(subplots=True, layout=(rows,4), figsize=(15, 10))
    plt.tight_layout()
    plt.savefig(os.path.join(config.RESULTS_PATH, f"{name}.png"))

def compute_single_trajectory_metrics(tensordict_data):
    tensordict_data["next", "done"] = tensordict_data["next", "agents", "done"].squeeze().any(-1)
