...
from rlkit.envs.transforms import append_batch_transform, InvertibleCatTensors, RenameAction, SoftResetWrapper, InvertibleRename
...
def create_base_walker_env(path=None, graphics=False, **kwargs):
    ...
    env = env.append_transform(
        InvertibleRename(prev_keys=env.observation_keys, new_keys=["observation"])
    )
    env = env.append_transform(
    ...
    env = env.append_transform(
        RewardScaling(loc=0.0, scale=100.0, standard_normal=True),
    )
    return env