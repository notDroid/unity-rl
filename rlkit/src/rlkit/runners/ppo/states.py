...
class PPOStateBuilder:
    def build(self, config: DictConfig, agent, loss_module) -> dict:
        ...
        self.ppo_sm = PPOStateManager(config.trainer.params, state)
        if config.get("continue_", True):
            self._restore_state(state)
        else:
            self.ppo_sm.reset_state(state)
    ...
    def _build_optional_components(self, config, state):
        ...
        if 'lr_scheduler' in comp_map:
            lr_scheduler = instantiate(
                ...
                total_epochs=config.generations
            )
        ...
        if 'metric_module' in comp_map:
            ...
            components['metric_module'] = SimpleMetricModule()
    ...
    def _restore_state(self, state):
        ...
        if checkpointer:
            ...
            if checkpoint:
                return self.ppo_sm.restore_checkpoint(checkpoint)
            else:
    ...
    def _get_log_keys(self, config):
        if "keys" in config.state.logger:
            return list(config.state.logger.keys)
        log_keys = ppo_log_keys.copy()
        maybe_add_key(log_keys, "lr_scheduler" in config, 'lr')
        maybe_add_key(log_keys, config.loss.params.get("inverse_coef", 0) > 0, 'inverse_loss')
        return log_keys
