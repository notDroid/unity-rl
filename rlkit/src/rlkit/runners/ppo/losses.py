...
class PPOLossBuilder:
    ...
    def _make_ppo_loss_module(self, config, agent):
        # Assemble Parameters
        loss_params = todict(config.loss.loss_params) if config.loss.get('loss_params') else {}
        if config.loss.get('include_inverse') is not None: include_inverse = config.loss.include_inverse
        ...
        # Value Estimator
        value_estimator_params = todict(config.loss.value_estimator_params) if config.loss.get('value_estimator_params') else {}
        if config.loss.get('value_estimator'):
