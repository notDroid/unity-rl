...
class PPOLossBuilder:
    ...
    def _make_ppo_loss_module(self, config, agent):
        # Assemble Parameters
        loss_params = todict(config.loss.params)
        ...
        # Loss Module
        loss_cls = get_class(config.loss.loss_fn)
        loss_module = loss_cls(
            actor_network=agent.get_policy_operator(),
            value_network=agent.get_value_operator(),
            **({"inverse_network": agent.get_inverse_operator() } if include_inverse else {}),
            **loss_params
        )

        # Value Estimator
        value_estimator_params = todict(config.loss.get('value_estimator_params')) or {}
        if 'value_estimator' in config.loss:
            value_estimator_cls = get_class(config.loss.get('value_estimator'))
            loss_module.make_value_estimator(value_estimator_cls)
        else:
            # Default to GAE
            loss_module.make_value_estimator(
                ValueEstimators.GAE, 
                gamma=loss_params['gamma'], lmbda=loss_params['lmbda'], 
